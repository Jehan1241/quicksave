...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    ...
    return format(
      displayValue,
      `d ${!hideTime ? "MMM" : "MMMM"}, yyyy${!hideTime ? (use12HourFormat ? " hh:mm:ss a" : " HH:mm:ss") : ""}`
    );
    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger asChild>
        {renderTrigger ? (
          ...
          <div
            className={cn(
              "flex w-full cursor-pointer items-center h-10 ps-3 pe-1 font-normal border border-input rounded-md text-sm shadow-sm",
