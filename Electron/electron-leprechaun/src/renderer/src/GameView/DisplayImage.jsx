...
import AddScreenshotPopUp from './AddScreenshotPopUp'
...
function DisplayImage(props) {
  ...
  const [addScreenshotClicked, setAddScreenshotClicked] = useState(false)
  ...
  const addScreenshotClickHandler = () => {
    setAddScreenshotClicked(!addScreenshotClicked)
    console.log(addScreenshotClicked)
  }
      ...
      <div className="flex flex-col overflow-y-auto w-[60%] gap-1">
        ...
        <div className="flex justify-center h-[67vh]" onWheel={handleWheel}>
          <img
            ...
            src={'http://localhost:8080/screenshots' + screenshotsArray[selectedScreenShot]}
          />
        </div>
        {/* X-Scrollbar Div */}
        <div
          ...
          {screenshotsArray.map((item, index) => (
            <div key={index} className={`w-32 min-w-32`}>
              <button onClick={() => imageTileClickHandler(index)}>
                <img
                  src={'http://localhost:8080/screenshots' + screenshotsArray[index]}
                  className={`w-32 max-h-[4.5rem] ${index === selectedScreenShot ? 'border-2' : ''}`}
                />
              </button>
            </div>
          ))}
        ...
        <div className="flex flex-row justify-center mt-1 mb-2">
          ...
          <button onClick={addScreenshotClickHandler} className="mx-2">
            +
          </button>
        </div>
      </div>
      {addScreenshotClicked ? (
        <AddScreenshotPopUp addScreenshotClickHandler={addScreenshotClickHandler} />
      ) : null}
    </>
  )
}
